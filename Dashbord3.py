...
async def main():
    async with async_playwright() as p:
        ...
        # FIXED ULTIMATE Turbo Analytics Dashboard
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body {
            ...
            color: white; min-height: 100vh; font-size: 12px; padding: 8px;
        }
        ...
        .main-grid { 
            ...
            gap: 15px; 
            margin-bottom: 15px; 
            height: 450px; /* Fixed height to prevent overlap */
        }
        ...
        .metric-card { 
            ...
            border: 1px solid rgba(255, 255, 255, 0.1); min-height: 60px;
            display: flex; flex-direction: column; justify-content: center;
        }
        .metric-title { font-size: 0.7em; opacity: 0.8; margin-bottom: 3px; text-transform: uppercase; }
        .metric-value { font-size: 1em; font-weight: bold; margin-bottom: 3px; }
        ...
        .strategy-section { margin-bottom: 12px; flex-shrink: 0; }
        .strategy-title { 
        ...
        .arbitrage-alert { 
            ...
            font-weight: bold; text-align: center; animation: pulse 2s infinite; font-size: 0.8em;
        }
        @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.7; } }
        
        .strategy-confidence { 
            width: 100%; height: 15px; background: rgba(255, 255, 255, 0.1); 
            border-radius: 8px; overflow: hidden; margin: 4px 0;
        }
        .confidence-fill { 
            height: 100%; border-radius: 8px; transition: all 0.5s ease;
            background: linear-gradient(90deg, #ff4444 0%, #ffaa00 50%, #00ff88 100%);
        }
        
        /* COMPARATIVE ANALYTICS - FIXED HEIGHT */
        .comparison-grid { 
        ...
        .main-chart h3 { 
            margin: 0 0 10px 0; color: #60a5fa; font-size: 1em; 
            flex-shrink: 0;
        }
        ...
        .mini-chart h4 { 
            margin: 0 0 5px 0; font-size: 0.8em; 
            flex-shrink: 0;
        }
...
<body>
    <div class="dashboard">
        ...
        <div class="main-grid">
            ...
            <div class="strategy-panel">
                ...
                <div class="strategy-section">
                    ...
                    <div style="text-align: center; font-size: 0.75em; margin-top: 3px;">
        ...
        <div class="charts-section">
            <div class="main-chart">
                <h3>📊 Comparative Turbo Analysis</h3>
            ...
            <div class="mini-charts">
                <div class="mini-chart">
                    <h4 style="color: #84cc16;">Spread Analysis</h4>
        ...
        <div class="data-panels">
            <div class="data-panel">
                <h3 style="color: #60a5fa;">📊 Live Turbo Data</h3>
                <div class="data-stream" id="turboDataStream"></div>
            </div>
            <div class="data-panel">
                <h3 style="color: #a855f7;">🧠 Strategy Events</h3>
                <div class="data-stream" id="strategyEvents"></div>
            </div>
            <div class="data-panel">
                <h3 style="color: #f59e0b;">💰 Trading Opportunities</h3>
                <div class="data-stream" id="tradingOpportunities"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let underlying = { prices: [], lastPrice: null, timestamps: [] };
        ...
        // Initialize charts with FIXED sizing to prevent overlaps
        function initCharts() {
            console.log('📊 Initializing FIXED turbo charts...');
            ...
            const commonOptions = {
                ...
                scales: {
                    x: {
                        ticks: { color: 'white', maxTicksLimit: 5, font: { size: 9 } },
            ...
            // MAIN CHART - Turbo Performance vs Underlying
            ...
            // Mini charts with simplified options
            const miniOptions = {
                responsive: true,
                maintainAspectRatio: false,
                devicePixelRatio: window.devicePixelRatio || 1,
            ...
            console.log('✅ All charts initialized with FIXED sizing');
        }
        ...
        // UPDATE TURBO ANALYTICS with PERCENTAGE displays
        function updateTurboAnalytics(type) {
            ...
            // Update UI with PERCENTAGE format where relevant
            const currentPrice = data.lastPrice;
            const priceBaseline = baseline[type] || currentPrice;
            const priceChangePercent = ((currentPrice - priceBaseline) / priceBaseline) * 100;
            
            document.getElementById(type + 'SMA').textContent = sma5 ? 
                (((sma5 - priceBaseline) / priceBaseline) * 100).toFixed(2) + '%' : '-';
            document.getElementById(type + 'RSI').textContent = rsi ? rsi.toFixed(1) : '-';
            document.getElementById(type + 'Volatility').textContent = volatility ? (volatility * 100).toFixed(2) + '%' : '-';
            ...
            if (volatility) {
                const volPercent = Math.min(volatility * 500, 100); // Scale volatility for display
            ...
            if (data.bids.length > 1 && data.asks.length > 1) {
                ...
                document.getElementById(type + 'VolumePressure').textContent = pressure.toFixed(1) + '%';
        ...
        function updateTurboChart(type) {
            ...
            // Convert to percentage data for display
            const maxPoints = 50; // Show last 50 data points
            const recentPrices = data.prices.slice(-maxPoints);
            const recentBids = data.bids.slice(-maxPoints);
            const recentAsks = data.asks.slice(-maxPoints);
            
            if (recentPrices.length === 0) return;
            
            // Calculate percentage changes from baseline
            const priceBaseline = baseline[type] || recentPrices[0];
            
            // Create labels and datasets
            const labels = Array.from({length: recentPrices.length}, (_, i) => (i + 1).toString());
            
            const priceData = recentPrices.map(price => ((price - priceBaseline) / priceBaseline) * 100);
            const bidData = recentBids.map(bid => bid && priceBaseline ? ((bid - priceBaseline) / priceBaseline) * 100 : 0);
            const askData = recentAsks.map(ask => ask && priceBaseline ? ((ask - priceBaseline) / priceBaseline) * 100 : 0);
        ...
        function updateMainChart() {
            ...
            const maxPoints = 40; // Show last 40 data points
            ...
            if (longPrices.length === 0 && shortPrices.length === 0) return;
            
            // Calculate percentage changes from baselines
            const longBaseline = baseline.long || (longPrices.length > 0 ? longPrices[0] : 1);
            const shortBaseline = baseline.short || (shortPrices.length > 0 ? shortPrices[0] : 1);
            const underlyingBaseline = baseline.underlying || (underlyingPrices.length > 0 ? underlyingPrices[0] : 1);
            
            // Create labels and datasets
            const maxLength = Math.max(longPrices.length, shortPrices.length, underlyingPrices.length);
            const labels = Array.from({length: maxLength}, (_, i) => (i + 1).toString());
            
            const longData = longPrices.map(price => ((price - longBaseline) / longBaseline) * 100);
            const shortData = shortPrices.map(price => ((price - shortBaseline) / shortBaseline) * 100);
            const underlyingData = underlyingPrices.map(price => ((price - underlyingBaseline) / underlyingBaseline) * 100);
        ...
        function updateMiniCharts() {
            // Update spread chart
            if (charts.spread && (turboData.long.spreads.length > 0 || turboData.short.spreads.length > 0)) {
                const maxPoints = 15;
            ...
            if (charts.volatility && (turboData.long.volatility.length > 0 || turboData.short.volatility.length > 0)) {
                const maxPoints = 15;
                const labels = Array.from({length: maxPoints}, (_, i) => (i + 1).toString());
            ...
            if (charts.momentum && (turboData.long.momentum.length > 0 || turboData.short.momentum.length > 0)) {
                const maxPoints = 15;
                const labels = Array.from({length: maxPoints}, (_, i) => (i + 1).toString());
        ...
        // PROPER STRATEGY AND SIGNAL GENERATION
        function generateTradingSignals() {
            const newSignals = [];
            
            // RSI-based entry signals for each turbo individually
            if (turboData.long.rsi.length > 0) {
                const longRSI = turboData.long.rsi[turboData.long.rsi.length - 1];
                const longMomentum = turboData.long.momentum.length > 0 ? turboData.long.momentum[turboData.long.momentum.length - 1] : 0;
                
                if (longRSI < 30 && longMomentum > 0) {
                    newSignals.push({
                        type: 'LONG_ENTRY',
                        signal: 'LONG oversold with positive momentum',
                        strength: 'ENTRY',
                        confidence: 75,
                        timestamp: Date.now()
                    });
                ...
                if (longRSI > 70 && longMomentum < 0) {
                    newSignals.push({
                        type: 'LONG_EXIT',
                        signal: 'LONG overbought with negative momentum',
                        strength: 'EXIT',
                        confidence: 75,
                        timestamp: Date.now()
                    });
            ...
            if (turboData.short.rsi.length > 0) {
                const shortRSI = turboData.short.rsi[turboData.short.rsi.length - 1];
                const shortMomentum = turboData.short.momentum.length > 0 ? turboData.short.momentum[turboData.short.momentum.length - 1] : 0;
                
                if (shortRSI < 30 && shortMomentum > 0) {
                    newSignals.push({
                        type: 'SHORT_ENTRY',
                        signal: 'SHORT oversold with positive momentum',
                        strength: 'ENTRY',
                        confidence: 75,
                        timestamp: Date.now()
                    });
                ...
                if (shortRSI > 70 && shortMomentum < 0) {
                    newSignals.push({
                        type: 'SHORT_EXIT',
                        signal: 'SHORT overbought with negative momentum',
                        strength: 'EXIT',
                        confidence: 75,
                        timestamp: Date.now()
                    });
            ...
            // Spread efficiency signals
            const longSpread = turboData.long.spreads.length > 0 ? turboData.long.spreads[turboData.long.spreads.length - 1] : 0;
            const shortSpread = turboData.short.spreads.length > 0 ? turboData.short.spreads[turboData.short.spreads.length - 1] : 0;
            
            if (longSpread > 0 && shortSpread > 0) {
                if (longSpread < shortSpread * 0.5) {
                    newSignals.push({
                        type: 'SPREAD_OPPORTUNITY',
                        signal: 'LONG has much lower spread than SHORT',
                        strength: 'OPPORTUNITY',  
                        confidence: 80,
                        timestamp: Date.now()
                    });
                } else if (shortSpread < longSpread * 0.5) {
                    newSignals.push({
                        type: 'SPREAD_OPPORTUNITY',
                        signal: 'SHORT has much lower spread than LONG',
                        strength: 'OPPORTUNITY',
                        confidence: 80,
                        timestamp: Date.now()
                    });
        ...
        function detectArbitrageOpportunities() {
            ...
            if (longSpread > 0 && shortSpread > 0) {
                const spreadDiff = Math.abs(longSpread - shortSpread);
                if (spreadDiff > 0.2) { // 0.2% spread difference
                    const cheaperTurbo = longSpread < shortSpread ? 'LONG' : 'SHORT';
                    const savings = spreadDiff;
                    
                    addTradingOpportunity(`💰 SPREAD arbitrage: ${cheaperTurbo} has ${savings.toFixed(2)}% lower spread`);
                }
            }
            ...
            // Spread efficiency display
            if (longSpread > 0 && shortSpread > 0) {
                const efficiency = longSpread < shortSpread ? `Long cheaper by ${(shortSpread - longSpread).toFixed(2)}%` : `Short cheaper by ${(longSpread - shortSpread).toFixed(2)}%`;
                document.getElementById('spreadEfficiency').textContent = efficiency;
            }
        }
        ...
        function resetEverything() {
            ...
            // Reset all data structures with timestamps
            turboData = {
                long: { 
                    prices: [], bids: [], asks: [], spreads: [],
                    sma5: [], sma20: [], ema10: [], rsi: [], momentum: [],
            ...
            underlying = { prices: [], lastPrice: null, timestamps: [] };
            ...
            const resetElements = [
                'longPrice', 'longBid', 'longAsk', 'longSpread', 'longVolatility', 'longMomentum', 'longRSI', 'longSMA', 'longVolumePressure',
                'shortPrice', 'shortBid', 'shortAsk', 'shortSpread', 'shortVolatility', 'shortMomentum', 'shortRSI', 'shortSMA', 'shortVolumePressure',
                'longShortRatio', 'correlation', 'spreadDivergence', 'volatilityRatio'
            ];
            ...
            // Reset change displays
            const changeElements = [
                'longChange', 'shortChange', 'longRSISignal', 'shortRSISignal', 'longTrend', 'shortTrend',
                'longPressureDir', 'shortPressureDir', 'ratioTrend', 'correlationStrength', 'divergenceSignal', 'volRatioSignal'
            ];
            ...
            const momentumElements = ['longMomentumInd', 'shortMomentumInd'];
            momentumElements.forEach(id => {
                const element = document.getElementById(id);
                if (element) {
                    element.textContent = 'FLAT';
                    element.className = 'momentum-indicator momentum-flat';
                }
            });
            
            // Reset volatility bars
            const volBars = ['longVolBar', 'shortVolBar'];
            volBars.forEach(id => {
                const element = document.getElementById(id);
            ...
            // Reset confidence
            document.getElementById('strategyConfidence').style.width = '0%';
            document.getElementById('confidenceText').textContent = 'Building confidence...';
            
            // Reset arbitrage display
            document.getElementById('arbitrageAlert').style.display = 'none';
            document.getElementById('longShortArb').textContent = 'Monitoring...';
            document.getElementById('spreadEfficiency').textContent = 'Calculating...';
            document.getElementById('breakoutSignal').textContent = 'Waiting...';
            document.getElementById('meanReversionSignal').textContent = 'Analyzing...';
            
            // Reset signals display
            const signalsContainer = document.getElementById('currentSignals');
            signalsContainer.innerHTML = '<div class="signal-item signal-neutral"><span>System:</span><span>Reset Complete</span></div>';
            ...
            const dataStreams = ['turboDataStream', 'strategyEvents', 'tradingOpportunities'];
            dataStreams.forEach(streamId => {
            ...
            // Add reset confirmation to streams
            addTurboDataStream('🔄 SYSTEM RESET - All data cleared');
            addStrategyEvent('🔄 Complete reset - Ready for new data');
            addTradingOpportunity('🔄 Reset complete - Monitoring for opportunities');
        ...
        try:
            print("📊 Loading FIXED ULTIMATE turbo strategy dashboard...")
            ...
            print(f"📊 FIXED dashboard functions ready: {functions_ready}")
            
            if functions_ready:
                print("✅ FIXED ULTIMATE TURBO ANALYTICS READY!")
                print("🎯 Charts should now be CLEAR and NON-OVERLAPPING!")
            else:
                print("❌ Dashboard not fully ready - check console")
                
        except Exception as e:
            print(f"❌ Error loading fixed dashboard: {e}")
        ...
        print("\n🎯 FIXED ULTIMATE TURBO STRATEGY ANALYTICS ACTIVE:")
        ...
        print("🔧 FIXED: Charts are now properly sized and non-overlapping!")
...
async def update_dashboard_underlying(price):
    ...
    try:
        if len(pages) >= 1:
            ...
            print(f"📊 ✅ FIXED underlying: {price}")
    except Exception as e:
...
async def handle_sse_response(response, page_name):
    ...
    try:
        if "streaming/sse" in response.url and response.status == 200:
            print(f"[{page_name}] 🎯 FIXED SSE Response detected")
    except Exception as e:
        pass
    ...
    try:
        ...
        ultimate_data = {
            ...
            "export_metadata": {
                ...
                "data_version": "fixed_ultimate_v1.0",
        ...
        filename = f"fixed_ultimate_turbo_strategy_data_{timestamp}.json"
        
        # Save comprehensive data
        with open(filename, 'w') as f:
            json.dump(ultimate_data, f, indent=2)
        
        print(f"\n💾 ✅ FIXED ULTIMATE TURBO DATA SAVED: {filename}")
        ...
        print(f"\n🚀 PERFECT FOR ALGORITHM DEVELOPMENT WITH FIXED UI!")
...
if __name__ == "__main__":
    print("🚀 FIXED ULTIMATE TURBO STRATEGY ANALYTICS")
    print("=" * 50)
